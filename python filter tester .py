...
def load_filters(path='lottery_filters_batch10.csv'):
    ...
    with open(path, newline='', encoding='utf-8') as f:
        ...
        for raw in reader:
            ...
            # keep-range filters: rescue combos inside range, eliminate outside
...
for combo in combos:
    ...
    ctx = dict(seed_digits=seed_digits, combo_digits=cd,
               seed_sum=sum(seed_digits), combo_sum=sum(cd),
               seed_counts=seed_counts, seed_vtracs=seed_vtracs,
               combo_vtracs=set(V_TRAC_GROUPS[d] for d in cd),
