...
def sum_category(s: int) -> str:
    """Categorize sum into sum buckets."""
...
def load_filters(path='lottery_filters_batch10.csv'):
    ...
    with open(path, newline='', encoding='utf-8') as f:
        ...
        for raw in reader:
            ...
            # Sum-category-transition filters (optional manual filters)
            ...
            # Last-three-seeds pattern filters
                ...
                if m:
                    ...
                    patt_list = [p.strip().strip("'") for p in pats_raw.split(',')]
...
def main():
    ...
    # Build prev_pattern from three seeds
    prev_pattern = []
    ...
    if check_combo:
        if check_combo in eliminated:
            st.sidebar.info(f"Combo {check_combo} eliminated by {eliminated[check_combo]}")
        elif check_combo in survivors:
    ...
    for flt in filters:
        count=sum(1 for combo in combos if eval(flt['applicable_code'],generate_context([int(c) for c in combo]),generate_context([int(c) for c in combo])) and eval(flt['expr_code'],generate_context([int(c) for c in combo]),generate_context([int(c) for c in combo])))
